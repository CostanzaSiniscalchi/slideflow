...
CONCH v1.5 is the patch encoder component of TITAN, designed for 448x448 pixel patches
at 20x magnification (though it can accept other sizes via interpolation).

The implementation is adapted from the official HuggingFace TITAN repository.
"""
...
class ConchConfig:
    """Configuration for CONCH v1.5 model."""
    def __init__(
        self,
        patch_size: int = 16,
...
def build_conch(checkpoint_path: str, img_size: int = 448):
    """Build CONCH v1.5 model from checkpoint.

    Args:
        checkpoint_path: Path to the conch_v1_5_pytorch_model.bin file
        img_size: Image size (default 448, can also use 512)

    Returns:
        model: The CONCH v1.5 encoder model
        eval_transform: Transform for preprocessing images
    """
    ...
    # CRITICAL: Use ImageNet normalization, NOT CLIP normalization
...
class ConchV15Features(TorchFeatureExtractor):
    ...
    computational pathology). This implementation uses the standalone CONCH model
    loaded from the downloaded checkpoint file.
...
"""
    ...
    def __init__(self, checkpoint_path: str, img_size: int = 448, device=None, **kwargs):
        ...
        Args:
            checkpoint_path: Path to conch_v1_5_pytorch_model.bin file
            img_size: Image size (448 or 512). Default is 448 as used in paper.
        ...
        # For Slideflow integration: we need to handle the transforms ourselves
        # since Slideflow provides uint8 tensors in WHC format
        # The eval_transform expects PIL images, so we adapt it

        # Build a transform that works with Slideflow's uint8 tensors
        # Slideflow provides images in WHC format (channels last)
        self.transform = T.Compose([
            T.Lambda(lambda x: x.permute(2, 0, 1) if x.shape[-1] == 3 else x),  # WHC -> CHW
            T.Lambda(lambda x: x.float() / 255.0),  # uint8 -> float [0, 1]
